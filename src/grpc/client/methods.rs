...
impl ChordClient {
    ...
    pub fn update_node(&mut self, node: Node, mask: FieldMask) -> impl Future<Item=Node, Error=ClientError> {
        let req = UpdateNodeRequest {
            node: Some(node),
            update_mask: Some(mask),
        };
        self.client.update_node(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| resp.into_inner())
    }
}
...
impl ChordClient {
    ...
    pub fn list_keys(&mut self) -> impl Future<Item=ListKeysResponse, Error=ClientError> {
        ...
        self.client.list_keys(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| resp.into_inner())
    }
    ...
    pub fn get_key(&mut self, name: &str) -> impl Future<Item=KeyMeta, Error=ClientError> {
        ...
        self.client.get_key(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| resp.into_inner())
    }
    ...
    pub fn create_key(&mut self, key: Key) -> impl Future<Item=KeyMeta, Error=ClientError> {
        ...
        self.client.create_key(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| resp.into_inner())
    }

    pub fn update_key(&mut self, key: Key, mask: FieldMask) -> impl Future<Item=KeyMeta, Error=ClientError> {
        let req = UpdateKeyRequest {
            key: Some(key),
            update_mask: Some(mask),
        };
        self.client.update_key(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| resp.into_inner())
    }
    ...
    pub fn delete_key(&mut self, name: &str) -> impl Future<Item=(), Error=ClientError> {
        ...
        self.client.delete_key(Request::new(req))
            .map_err(ClientError::from)
            .map(|resp| ())
    }
}
